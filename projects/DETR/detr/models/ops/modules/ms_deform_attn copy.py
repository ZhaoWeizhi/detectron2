...
class MSDeformAttn(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        constant_(self.time_attention_weights.weight.data, 0.)
        constant_(self.time_attention_weights.bias.data, 0.)
    ...
    def encode_forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        # time_attention_weights = self.time_attention_weights(query)
        # time_attention_weights = F.interpolate(time_attention_weights.flatten(0,1), size=(nf), mode='linear', align_corners=True).reshape(N,nf, Len_q,-1)
        # time_attention_weights = F.softmax(time_attention_weights, -1)
        # import pdb;pdb.set_trace()
        ...
        for idx_f in range(nf):
            # import pdb;pdb.set_trace()
            sampling_offsets_i = sampling_offsets[:,idx_f]  # 第i帧的offset
            ...
            attention_weights_i = attention_weights[:,idx_f].contiguous() # 第i帧采样点对应的权重
            ...
            # time_attention_weights_i = time_attention_weights[:,idx_f]
            # atti = time_attention_weights_i.permute(0,2,1)[:,:,:,None].contiguous()  # 将最后插值的维度转移到dim=1 的time 维度上
            # result_idx_f = []   
            # for samp_i in range(nf): #
            #     output_samp_i = MSDeformAttnFunction.apply(
            #         value_list[samp_i], input_spatial_shapes, input_level_start_index, sampling_locations_i, attention_weights_i, self.im2col_step)
            #     result_idx_f.append(output_samp_i.unsqueeze(1))
            # result_idx_f = torch.cat(result_idx_f,dim=1) #第idx_f 帧对其余帧的采样结果，通过time weight 相加在一起
            # result_idx_f = (result_idx_f*atti).sum(1) # 与temporal attenion weights 相乘之后在时间维度相加
            # result_list.append(result_idx_f.unsqueeze(1))
    ...
    def decode_forward(self, query, query_box, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        if len(query_box.shape)==3: #first layer
            ...
            time_attention_weights = self.time_attention_weights(query)
            time_attention_weights = F.interpolate(time_attention_weights, size=(nf), mode='linear', align_corners=True)
            time_attention_weights = F.softmax(time_attention_weights, -1)
            ...
            atti = time_attention_weights.permute(0,2,1)[:,:,:,None].contiguous()  # 将最后插值的维度转移到dim=1 的time 维度上
            result_idx_f = []
            ...
            for samp_i in range(nf): # perform deformable attention per frame
                ...
                elif reference_points_i.shape[-1] == 4:  # 使用box refine 传入的是box_embedd 的输出
            ...
            result_idx_f = torch.cat(result_idx_f,dim=1) #第idx_f 帧对其余帧的采样结果，通过time weight 相加在一起
            # import pdb;pdb.set_trace()    
            result_sum = (result_idx_f*atti).sum(1) # 与temporal attenion weights 相乘之后在时间维度相加
            # result_sum = result_idx_f.mean(1)
